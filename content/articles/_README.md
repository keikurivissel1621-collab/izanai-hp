...
画像は **Pexels（無料API）から記事ごとに自動取得**します（キー無し時はOpenverse、失敗時は手持ち写真にフォールバック）。

- **サムネ**：`thumbnail: auto`（または省略）で自動取得。特定画像を使いたい時はパス指定（例 `/assets/hero/xxx.jpg`）
- **検索語**：`image_keywords: [英語キーワード, ...]` を指定（英語が精度高い）。サムネ＆挿絵に使われる
- **本文の挿絵**：
  - 自動 … プレースホルダが無い記事は、2番目・4番目の大見出し直後に自動挿入
  - 手動 … 本文に `![代替テキスト](auto:検索キーワード)` と書くと、その位置にその画像が入る
- 取得画像は `assets/blog-img/<slug>/` に保存（圧縮済み・コミット対象）。再取得は `--refresh-images`

### Pexels APIキー（高品質・推奨）
`scripts/pexels_key.txt`（Git管理外）にキーを書くだけ。未設定でもOpenverseで動きます。
